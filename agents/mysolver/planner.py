...
# 「評価側と候補構築側が同じunits/secになる」較正が崩れている(候補構築が相対的に軽くなった)。
# 同じ手法(tools/phase17_probe.py、旧6シーン、K=COST_CONST=8固定)で測り直したところ
#   評価 1.211e7 units/s ÷ 候補構築 1.401e6 生units/s = 8.64
# だった(Phase17時点は 1.63e7 ÷ 8.94e5 = 18.2)。
CANDIDATE_BUILD_COST = 8.64
...
#
# Phase19(ターゲット3)で CANDIDATE_BUILD_COST=8.64(上記)込みで測り直したところ、
# 旧6シーンの集計は 1.088e7 units/s(シーン別には 9.93e6〜1.145e7 の幅)だった
# (Phase17時点は集計1.60e7・シーン別1.31e7〜2.01e7)。ターゲット1・2で「同じ壁時計予算で
# 消化できる評価回数」自体は増えたが、候補構築側の生コスト縮小に伴い1回あたりの
# 名目ユニット数自体も下がったため、両者を打ち消し合った結果このunits/secは
# **Phase17時点より下がる**(=同じnominal秒予算に対する実所要時間はPhase17より長くなる
# 方向ではなく、両定数を同時に下げることで名目秒と実秒の対応をPhase17と同水準に保つ、
# という較正の意味であることに注意)。Phase17と同じ「集計よりわずかに小さい値」を採用する
# 方針で 1.05e7 を採用する。
