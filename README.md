...
### 4-6. 到達点: 到達不能候補の早期枝刈り+増分キャッシュ(Phase19)
...
Phase20・Phase21を通じて、「代理関数を正確にする」「計上率を上げる」という2つの迂回路がどちらも閉じたことになります。残るレバーは**置けた体積そのものを増やすこと**——つまり配置能力の向上です。Phase20で候補順序による実充填率のスプレッドが平均19.19ptと大きいことは測定済みなので、ここには余地が実在します。次はtrust region + ρ-test(代理を信じず候補ごとにgatingする設計)と、Phase19で得た高速化の余力を活かす探索構造の見直しを併せて検討します。
...
- **未解決の課題**: 旧6シーンの1つ(`gen_2containers_priority`)の安定性制約違反(Phase17以降未解決)、置けた体積を増やす探索構造の見直し(Phase19・Phase21からの申し送り)、trust region + ρ-test設計の実装。本番の内包判定レジーム(厳/緩)が未確定で、緩なら計上漏れは0.7%しかなくPhase21の分析対象そのものが消える点も継続課題です。
