...
本READMEは成果のハイライトです。各フェーズの詳細な仮説・実験・数値根拠は [`results/phase10_report.md`](results/phase10_report.md) 〜 [`results/phase29_report.md`](results/phase29_report.md) にすべて残しており、本文中の主張はすべてそこに1次データがあります(コードは評価基盤の性質上 `agents/mysolver/` を公開していますが、コンテスト公募要領そのものの転載は避けています)。
...
- **未解決の課題**: 旧6シーンの1つ(`gen_2containers_priority`)の安定性制約違反(Phase17以降未解決)、ビーム幅とリスタート回数の配分の再検討(Phase25aの基準でσを抑えつつ再挑戦する必要がある)、Phase23で悪化した5シーンとくにD01(−14.78)の原因特定。搬入経路の封鎖については、Phase24(候補側)・Phase28(目的関数側)・Phase29(探索の誘導側)の3回でいずれも不採用となり、**Phase29でそもそも順序修正の射程内に無い量だと確定した**ため、この方向は閉じました。代わりに残っているのは「エピソードを止めた荷物がそもそも入らない」13シーンの側(搬入経路ではなく容積配分の問題)と、Phase29で具体化した代理関数の弁別誤差(1手差の粒度で順位が入れ替わる)です。本番の内包判定レジーム(厳/緩)が未確定である点も継続課題です(ただしPhase27で、`env.py:51`の配線により採点と合法性判定が**常に同じマージンを共有する**ことが確定したため、`fill_strict`と`fill_loose`を独立に追える2つの目標として扱うのは誤りだと分かっています)。
