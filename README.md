...
```mermaid
xychart-beta
    title "提出スコアの推移(#12・#13は不採用だった変種、主枠は#11の57.18のまま)"
    x-axis ["baseline", "phase4", "phase6", "phase7", "phase8", "phase9", "phase15", "phase19", "phase25a", "phase70", "phase71", "phase73", "phase78/80"]
    y-axis "SIGNATE提出スコア" 0 --> 60
    line [16.94, 24.01, 31.26, 37.31, 40.18, 42.47, 50.25, 51.00, 53.64, 56.05, 57.18, 54.80, 55.68]
```

#12・#13はいずれも主枠(#11、緩2、public 57.18)に対する変種を個別提出して不採用と判定したもので、
**主枠を置き換えたわけではない**(SIGNATEには常に主枠+保険枠の2本を提出しており、上表・上グラフは
「その時点で最も良かった/検証した提出」の推移であって「常に採用され続けている構成の推移」ではない)。
Phase73〜81で試した支持閾値・幾何定数・順序制御(オンライン/オフライン双方)・ρ-testのレバーは
Phase80時点で棚卸しが完了し、配置数(`num_placed_items`)を増やす経路は実質的に尽きたと判断している
(詳細は[§6](#6-学んだこと今後の課題)、`results/phase80_report.md`)。
