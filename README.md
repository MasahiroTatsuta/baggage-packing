...
本READMEは成果のハイライトです。各フェーズの詳細な仮説・実験・数値根拠は [`results/phase10_report.md`](results/phase10_report.md) 〜 [`results/phase32_report.md`](results/phase32_report.md) にすべて残しており、本文中の主張はすべてそこに1次データがあります(コードは評価基盤の性質上 `agents/mysolver/` を公開していますが、コンテスト公募要領そのものの転載は避けています)。
...
- **未解決の課題**: 旧6シーンの1つ(`gen_2containers_priority`)の安定性制約違反(Phase17以降未解決)、ビーム幅とリスタート回数の配分の再検討(Phase25aの基準でσを抑えつつ再挑戦する必要がある)、Phase23で悪化した5シーンとくにD01(−14.78)の原因特定。搬入経路の封鎖(iii区分)については、Phase24(候補側)・Phase28(目的関数側)・Phase29(探索の誘導側)の3回でいずれも不採用となり、この方向は閉じました。Phase30で「エピソードを止めた荷物がそもそも入らない」13シーンをさらに分解した結果、その83%(体積の80.9%)が**幾何的にそもそも入らない**((A)区分、容積配分の問題)で、支持面が無いだけ((B)区分)は17%にとどまることが確定しています。21シーン全体の行き詰まり分類でもこの(A)=(i)がシーン数48%・残体積54.8%で最大区分であり、**Phase31の本命はここへ届くALNS**(破壊→再構築、除去範囲を明示的に選べるためPhase29の1手移動より広い破壊半径を持つ)としました。本番の内包判定レジーム(厳/緩)が未確定である点も継続課題です(ただしPhase27で、`env.py:51`の配線により採点と合法性判定が**常に同じマージンを共有する**ことが確定したため、`fill_strict`と`fill_loose`を独立に追える2つの目標として扱うのは誤りだと分かっています)。
...
- **選択則(risk_vol)側は打ち切り、ALNSへ(Phase31/32)**: 候補順序130件を本物の評価器で再評価した結果、選択則の取りこぼしは26シーン換算+2.638(オラクル上限)。risk_vol の「壁への近さ罰則」を床面限定に絞る案(t=1.177)、4指標のBorda順位和(t=−0.098)の2案を検証しましたがいずれも不採用でした。Phase32で不採用の構造的原因(4指標中3指標が独立でない=実質3対1の多数決)まで特定できたため、**単純な指標の置換・アンサンブルでこれ以上取れる伸びしろは薄いと判断**し、選択則側の追試は打ち切ってPhase30が確定させたALNS路線に進みます。
