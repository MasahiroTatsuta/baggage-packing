...
本READMEは成果のハイライトです。各フェーズの詳細な仮説・実験・数値根拠は [`results/phase10_report.md`](results/phase10_report.md) 〜 [`results/phase31_report.md`](results/phase31_report.md) にすべて残しており、本文中の主張はすべてそこに1次データがあります(コードは評価基盤の性質上 `agents/mysolver/` を公開していますが、コンテスト公募要領そのものの転載は避けています)。
...
- **選択則(risk_vol)側の未決着(Phase31、判断待ち)**: 候補順序130件を本物の評価器で再評価した結果、選択則の取りこぼしは26シーン換算+2.638(オラクル上限)。risk_vol の「壁への近さ罰則」を床面限定に絞る案を実装・A/B(t=1.177で不採用)しましたが、D01(softheavy)で−10.77の反例が出ました。原因は「壁際への罰則が、shadow simulatorの過大評価を偶発的に格下げする正則化としても機能していた」ため。次の一手(床面+背面への拡張/ALNSへ進む/追加診断)は次フェーズで判断します。
