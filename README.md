...
**代理目的関数と真の評価の乖離**が、現在向き合っている中心的な課題です。オフライン探索(`ordering.build_order`)が最適化しているのは影シミュレータ上の「risk調整済み体積」であり、本番の合成スコアそのものではありません。Phase17でこのギャップが単一シーンで再現可能な形になり(安定性制約違反として顕在化)、Phase18で「単一の大域重みでこの誤差を埋めようとする」試みが、シーンごとに崖の位置が異なるため破綻することを実証しました。Phase19では探索の高速化(最大29%)を達成したにもかかわらず、その余力がそのまま明確なfill改善には転化しなかった(シーンごとにプラス・マイナス両方向に振れる)ことも同根の兆候として記録しています。次の設計候補として、単一重みで全候補・全シーンを一律に割り引くのではなく、候補ごとに「代理指標の改善予測」と「実測に近い代理」の比でgatingするtrust region + ρ-test構成を検討していますが、まだ着手していません。
