...
### 次セッションの WBC 作業(優先順)
1. **密な壁充填**: 配置後の maximal-space 分割で生まれるスリットを活かす。`WBC_MIN_SPACE` を
   荷物実寸に応じて動的に(現行 0.02 固定)、`_pick_ref` の `min_item_dim` を「残り荷物の最小」でなく
   「その空間に入る最小の残り荷物」に。1壁あたりの目標個数をダンプで確認。
2. **A08(多数小物)**: init/split のスリット過剰生成 → `_prune` の包含判定を許容付きに、
   隣接する薄いスリットの併合(merge)を実装。
3. **B01 が貪欲縮退している理由**を1シーンダンプで特定(WBC プランが空/短い?)。
