...
旧6シーン(§2.3, §3.3で検証した5+6シーン)ではこの組み合わせでも一度も差が出なかったため、
旧6シーンだけの検証では発見できなかった。Phase16以来くり返し観測されている「探索
...
`tools/measure.py --repeats 1`(既定 optimize 予算120s)で、変更前(pre-phase19、
commit `4a5cbff`)と変更後(本フェーズ最終版)を比較した(`tools/compare_suite.py`)。
