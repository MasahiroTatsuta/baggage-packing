...
1. **ローカル評価の実行条件が本番と異なる**: ローカル計測は`MYSOLVER_HARD_WALL_LIMIT`を
   ...
   > **【Phase45/46による訂正】** この段落の前提(「ローカルは余裕のある実行=完走に近い」)は
   > 誤りだった。Phase45で確認した結果、**ローカルも25/26シーンが本番と同じ
   > "Stopped in the middle" で途中終了しており、平均配置率52.3%は本番の61.4%より
   > むしろ低い**。また Phase45 ステップ2で本番既定の壁時計拘束(165秒)を課しても
   > 26シーン全件で挙動が非拘束時と完全に同一だったため、**この意味での「壁時計の圧力」
   > 仮説はローカルでは検証すらできなかった**(ローカル環境ではそもそも165秒の天井に
   > 一度も到達しない)。誤りに気づいた記録として、この段落は削除せずそのまま残す。
   > 詳細は `results/phase45_report.md`。
2. **本番の`placement_score`/`soft_item_score`の判定基準がローカル近似(tools/scorer.py)と
   ...
   > **【Phase46による補足】** Phase46 で、ソフト貨物比率30/50/70%・優先手荷物比率
   > 30/50/70%まで人工的に引き上げた検証シーン(実測では最大 soft比率80%・prio比率62%)
   > でも `_find_stacking_pairs` ベースの違反は依然0件だった。plannerが「違反しようが
   > ない構成だから」ではなく「本当に違反を回避できている」ことが追加で裏付けられ、
   > **消去法として本項(判定基準そのものの食い違い)が最有力**になった。
   > 詳細は `results/phase46_report.md`。
